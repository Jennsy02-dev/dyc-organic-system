...
export const generateHairDiagnosis = async (hairType: string, scalpCondition: string, mainProblem: string) => {
  try {
    const prompt = `
      ...
      Genera una rutina capilar recomendada usando productos 100% orgánicos, explicando los beneficios de ingredientes naturales como el romero, jengibre, keratina vegetal o aceite de argán. Sé amable, profesional y directo.
    `;
